...
if (!isset($opensale[0]['id'])) {
    ...
    $repetido = $pdo->prepare("SELECT productos_id, cantidad FROM detalleventa where ventas_id = $idsale2 and productos_id = $id;");
    ...
    if (empty($repetidos)) {
        ...
        $producto = $pdo->prepare("INSERT INTO `detalleventa` ( `cantidad`, `productos_id`, `talla`, `ventas_id`) VALUES ('1', '$id',  '', '$idsale2');");
        $producto -> execute();

        $precio = $pdo->prepare("SELECT * FROM productos where id = '$id'");
        $precio -> execute();
        $currentProduct=$precio->fetchAll(PDO::FETCH_ASSOC);

        $subtotal = $currentProduct[0]['precio_venta'];

        $venta = $pdo->prepare("UPDATE ventas set subtotal = $subtotal where id = $idsale2");
        $venta -> execute();

        if (empty($iduser)) {
            header("location:../login/login.php");
        }else{
        header("location:../carrito de compras/pagos1.php");
        }
    }else{

        $cantidad = $repetidos[0]['cantidad'] + 1;

        $repetido_sum = $pdo->prepare("UPDATE detalleventa SET cantidad = $cantidad where productos_id = $id and ventas_id = $idsale2;");
    ...
    $repetido = $pdo->prepare("SELECT productos_id, cantidad, id FROM detalleventa where ventas_id = $idsale and productos_id = $id;");
    ...
    if (empty($repetidos)) {
        ...
        $producto = $pdo->prepare("INSERT INTO `detalleventa` ( `cantidad`, `productos_id`,  `talla`, `ventas_id`) VALUES ('1', '$id',  '', '$idsale');");
        $producto -> execute();
        ...
        $subtotal = $currentProduct[0]['precio_venta'] + $opensale[0]['subtotal'];

        $venta = $pdo->prepare("UPDATE ventas set subtotal = $subtotal where id = $idsale");
        $venta -> execute();
